...
export class PatientsTableComponent implements OnInit {
	...
	search(filter: string) {
		if (filter.length == 0) {
			...
			this.patientList = this.patientList.filter((patient: any) => patient.name.toUpperCase()
				.includes(filter.toUpperCase()) || patient.email?.includes(filter))
		}
	}
	...
	onDeactivatePatient(patient: Patient) {
		...
		if (confirmation) {
			...
			this.deactivatePatientsExercises(patient)
	...
	deactivatePatientsExercises(patient: Patient) {
		this.exerciseService.getExercisesByPatient(patient.id.toString()).subscribe({
			next: (exercises: any) => {
				console.log(exercises)
				if (exercises && exercises.length > 0) {
					exercises.forEach((exercise: any) => {
						this.exerciseService.updateExercise({...exercise, status: false}).subscribe()
					})
				}
			}
		})
